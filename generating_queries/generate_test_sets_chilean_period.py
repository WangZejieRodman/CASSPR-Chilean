...
def construct_query_and_database_sets_separate(base_path, runs_folder,
    ...
    Args:
        database_folders: 数据库session列表 (100-179)
        query_folders: 查询session列表 (180-209)
    """
...
runs_folder = "chilean_原始点云-非4096点云/"
...
for folder in valid_folders:
    ...
    if DATABASE_SESSION_START <= session_num <= DATABASE_SESSION_END:
        database_folders.append(folder)  # 100-179作为数据库
    elif QUERY_SESSION_START <= session_num <= QUERY_SESSION_END:
        query_folders.append(folder)  # 180-209作为查询
...
database_sets, query_sets = construct_query_and_database_sets_separate(
    base_path,
    runs_folder,
    database_folders,  # 数据库用100-179
    query_folders,  # 查询用180-209
    pointcloud_fols,
...
print(f"Database (100-179): Historical point clouds serving as reference map")
print(f"Query (180-209): Current observations for localization")
