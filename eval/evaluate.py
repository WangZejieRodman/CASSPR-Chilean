...
def load_pc(filename, params):
    ...
    # 修改路径处理逻辑
    if params.dataset_name == "Chilean":
        file_path = os.path.join('/home/wzj/pan2/Chilean_Underground_Mine_Dataset_Many_Times', filename)
    else:
        # 其他数据集使用原来的路径
    ...
    elif params.dataset_name == "Chilean":
        pc = np.fromfile(file_path, dtype=np.float64).reshape([-1, 3])
...
if __name__ == "__main__":
    ...
    class Args:
        def __init__(self):
            # 根据你的需求修改这些路径
            self.config = '../config/config_oxford.txt'
            self.model_config = '../config/model_config_oxford.txt'
            self.weights = '/home/wzj/pan1/CASSPR/pretrained/CASSPR-Oxford.pth'# 训练好的模型权重路径，如果没有则使用随机权重
            self.debug = False
    ...
    w_list = w.split('/')
    logdir = '/'.join(w_list[:-1]) if args.weights else './'
    epoch = w_list[-1].split('.')[0] if args.weights else 'random'
